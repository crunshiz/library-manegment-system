...
class Management:
    def __init__(self, root):
        ...
        self.frame_1 = Frame(self.window, bg=cs.white)
        self.frame_1.place(x=0, y=0, width=740, relheight=1)


        self.frame_2 = Frame(self.window, bg=cs.gray)
        self.frame_2.place(x=740, y=0, relwidth=1, relheight=1)


        self.frame_3 = Frame(self.frame_2, bg=cs.gray)
        self.frame_3.place(x=0, y=300, relwidth=1, relheight=1)


        self.add_book = Button(self.frame_2, text='Add Book', font=(cs.tnr, 12), bd=2, command=self.AddNewBook,
                               cursor="hand2", bg=cs.gray, fg=cs.black).place(x=50, y=40, width=100)
        self.issue_book = Button(self.frame_2, text='Issue Book', font=(cs.tnr, 12), bd=2,
                                 command=self.GetData_for_IssueBook, cursor="hand2", bg=cs.gray,
                                 fg=cs.black).place(x=50, y=100, width=100)
        self.return_book = Button(self.frame_2, text='Return Book', font=(cs.tnr, 12), bd=2, command=self.ReturnBook,
                                  cursor="hand2", bg=cs.gray, fg=cs.black).place(x=50, y=160, width=100)
        self.all_book = Button(self.frame_2, text='All Books', font=(cs.tnr, 12), bd=2, command=self.ShowBooks,
                               cursor="hand2", bg=cs.gray, fg=cs.black).place(x=50, y=220, width=100)

        self.search_book = Button(self.frame_2, text='Search Book', font=(cs.tnr, 12), bd=2,
                                  command=self.GetBookNametoSearch, cursor="hand2", bg=cs.gray, fg=cs.black).place(
            x=180, y=40, width=100)
        self.all_borrow_records = Button(self.frame_2, text='Book Holders', font=(cs.tnr, 12), bd=2,
                                         command=self.AllBorrowRecords, cursor="hand2", bg=cs.gray,
                                         fg=cs.black).place(x=180, y=100, width=100)
        self.clear = Button(self.frame_2, text='Clear Screen', font=(cs.tnr, 12), bd=2, command=self.ClearScreen,
                            cursor="hand2", bg=cs.gray, fg=cs.black).place(x=180, y=160, width=100)
        self.exit = Button(self.frame_2, text='Exit', font=(cs.tnr, 12), bd=2, command=self.Exit, cursor="hand2",
                           bg=cs.gray, fg=cs.black).place(x=180, y=220, width=100)


    def OnSelectedforReturn(self, a):
        self.dlt_record = Button(self.frame_3, text='Delete', font=(cs.tnr, 12), bd=2, command=self.ReturningBook,
                                 cursor="hand2", bg=cs.gray, fg=cs.black).place(x=50, y=0, width=100)
        self.borrow_again = Button(self.frame_3, text='Issue Again', font=(cs.tnr, 12), bd=2,
                                   command=self.IssueAgain, cursor="hand2", bg=cs.gray, fg=cs.black).place(x=180,
                                                                                                                y=0,
                                                                                                                width=100)
    def OnSelectedforBorrowRecords(self, a):
        self.dlt_record = Button(self.frame_3, text='Delete', font=(cs.tnr, 12), bd=2, command=self.ReturningBook,
                                 cursor="hand2", bg=cs.gray, fg=cs.black).place(x=50, y=0, width=100)
        self.update_record = Button(self.frame_3, text='Update', font=(cs.tnr, 12), bd=2, command=self.IssueAgain,
                                    cursor="hand2", bg=cs.gray, fg=cs.black).place(x=180, y=0, width=100)

    def OnSelectedforShowBooks(self, a):
        self.dlt_record = Button(self.frame_3, text='Delete', font=(cs.tnr, 12), bd=2, command=self.DeleteBook,
                                 cursor="hand2", bg=cs.gray, fg=cs.black).place(x=50, y=0, width=100)
        self.update_record = Button(self.frame_3, text='Update', font=(cs.tnr, 12), bd=2,
                                    command=self.UpdateBookDetails, cursor="hand2", bg=cs.gray, fg=cs.black).place(
            x=180, y=0, width=100)
    ...
    def IssueAgain(self):
        ...
        book_id = Label(self.frame_1, text="Book Id", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=130, y=30)
        id = Label(self.frame_1, text=row[0], font=(cs.tnr, 10))
        id.place(x=130, y=60, width=200)

        book_name = Label(self.frame_1, text="Book Name", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=400,
                                                                                                             y=30)
        bookname = Label(self.frame_1, text=row[1], font=(cs.tnr, 10))
        bookname.place(x=400, y=60, width=200)

        student_roll = Label(self.frame_1, text="Student Roll", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(
            x=130, y=100)
        sturoll = Label(self.frame_1, text=row[2], font=(cs.tnr, 10))
        sturoll.place(x=130, y=130, width=200)

        student_name = Label(self.frame_1, text="Student Name", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(
            x=400, y=100)
        stuname = Label(self.frame_1, text=row[3], font=(cs.tnr, 10))
        stuname.place(x=400, y=130, width=200)

        course = Label(self.frame_1, text="Course", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=130, y=170)
        Course = Label(self.frame_1, text=row[4], font=(cs.tnr, 10))
        Course.place(x=130, y=200, width=200)

        subject = Label(self.frame_1, text="Subject", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=400, y=170)
        Subject = Label(self.frame_1, text=row[5], font=(cs.tnr, 10))
        Subject.place(x=400, y=200, width=200)

        issue_date = Label(self.frame_1, text="Issue Date", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=130,
                                                                                                               y=240)
        issuedate = Label(self.frame_1, text=row[6], font=(cs.tnr, 10))
        issuedate.place(x=130, y=270, width=200)

        return_date = Label(self.frame_1, text="Return Date", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=400,
                                                                                                                 y=240)
        self.return_date_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.return_date_entry.insert(0, row[7])
        self.return_date_entry.place(x=400, y=270, width=200)

        self.submit_bt = Button(self.frame_1, text='Submit', font=(cs.tnr, 12), bd=2,
                                command=partial(self.BorrowBookAgain, row), cursor="hand2", bg=cs.gray,
                                fg=cs.black).place(x=300, y=320, width=100)
    ...
    def ReturnBook(self):
        self.ClearScreen()
        return_book = Label(self.frame_1, text="Return Book", font=(cs.tnr, 30, "bold"), bg=cs.white).place(x=250,
                                                                                                                 y=40)
        roll_no = Label(self.frame_1, text="Enter the Roll No.", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(
            x=210, y=140)
        self.roll_no_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.roll_no_entry.place(x=210, y=175, width=300)

        self.search_bt = Button(self.frame_1, text='Search', font=(cs.tnr, 12), bd=2,
                                command=self.ShowRecordsforReturn, cursor="hand2", bg=cs.gray, fg=cs.black).place(
    ...
    def UpdateBookDetails(self):
        ...
        book_id = Label(self.frame_1, text="Book Id", font=(cs.helvetica, 18, "bold"), bg=cs.white).place(x=220, y=30)
        id = Label(self.frame_1, text=row[0], font=(cs.tnr, 10))
        id.place(x=220, y=60, width=300)

        book_name = Label(self.frame_1, text="Book Name", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=220,
                                                                                                             y=100)
        self.bookname_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.bookname_entry.insert(0, row[1])
        self.bookname_entry.place(x=220, y=130, width=300)

        author = Label(self.frame_1, text="Author", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=220, y=170)
        self.author_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.author_entry.insert(0, row[2])
        self.author_entry.place(x=220, y=200, width=300)

        edition = Label(self.frame_1, text="Edition", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=220, y=240)
        self.edition_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.edition_entry.insert(0, row[3])
        self.edition_entry.place(x=220, y=270, width=300)

        price = Label(self.frame_1, text="Price", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=220, y=310)
        self.price_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.price_entry.insert(0, row[4])
        self.price_entry.place(x=220, y=340, width=300)

        quantity = Label(self.frame_1, text="Quantity", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=220, y=380)
        self.qty_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.qty_entry.insert(0, row[5])
        self.qty_entry.place(x=220, y=410, width=300)

        self.submit_bt_1 = Button(self.frame_1, text='Submit', font=(cs.tnr, 12), bd=2,
                                  command=partial(self.SubmitforUpdateBook, row), cursor="hand2", bg=cs.gray,
                                  fg=cs.black).place(x=310, y=459, width=100)
    ...
    def GetData_for_IssueBook(self):
        ...
        book_id = Label(self.frame_1, text="Book Id", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=130, y=30)
        self.book_id_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.book_id_entry.place(x=130, y=60, width=200)

        book_name = Label(self.frame_1, text="Book Name", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=400,
                                                                                                             y=30)
        self.book_name_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.book_name_entry.place(x=400, y=60, width=200)

        student_roll = Label(self.frame_1, text="Student Roll", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(
            x=130, y=100)
        self.stu_roll_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.stu_roll_entry.place(x=130, y=130, width=200)

        student_name = Label(self.frame_1, text="Student Name", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(
            x=400, y=100)
        self.stu_name_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.stu_name_entry.place(x=400, y=130, width=200)

        course = Label(self.frame_1, text="Course", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=130, y=170)
        self.course_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.course_entry.place(x=130, y=200, width=200)

        subject = Label(self.frame_1, text="Subject", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=400, y=170)
        self.subject_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.subject_entry.place(x=400, y=200, width=200)

        issue_date = Label(self.frame_1, text="Issue Date", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=130,
                                                                                                               y=240)
        self.issue_date_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        ...
        self.submit_bt_1 = Button(self.frame_1, text='Submit', font=(cs.tnr, 12), bd=2,
                                  command=self.Submit_borrow_data, cursor="hand2", bg=cs.gray, fg=cs.black).place(
            x=300, y=320, width=100)


    def GetBookNametoSearch(self):
        self.ClearScreen()
        search_book = Label(self.frame_1, text="Search Book", font=(cs.tnr, 30, "bold"), bg=cs.white).place(x=250,
                                                                                                                 y=40)
        book_name = Label(self.frame_1, text="Enter the Book Name", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(
            x=220, y=140)
        self.book_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        self.book_entry.place(x=220, y=175, width=300)

        self.search_bt = Button(self.frame_1, text='Search', font=(cs.tnr, 12), bd=2, command=self.SearchBook,
                                cursor="hand2", bg=cs.gray, fg=cs.black).place(x=310, y=215, width=100)
    ...
    def ShowBooks(self):
        ...
        self.tree = ttk.Treeview(self.frame_1, columns=cs.columns_2, height=400, selectmode="extended",
    ...
    def AddNewBook(self):
        ...
        book_id = Label(self.frame_1, text="Book Id", font=(cs.helvetica, 15, "bold"), bg=cs.white).place(x=220, y=30)
        self.id_entry = Entry(self.frame_1, bg=cs.white, fg=cs.black)
        ...
        self.submit_bt_1 = Button(self.frame_1, text='Submit', font=(cs.tnr, 12), bd=2, command=self.Submit,
                                  cursor="hand2", bg=cs.gray, fg=cs.black).place(x=310, y=459, width=100)
